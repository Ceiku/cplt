...
#[test]
fn profile_allow_unix_socket_connect_only() {
    let sock = std::path::PathBuf::from("/tmp/agent-broker.sock");
    let p = generate_profile(&ProfileOptions {
        project_dir: std::path::Path::new("/tmp/proj"),
        home_dir: std::path::Path::new("/Users/test"),
        extra_read: &[],
        extra_write: &[],
        extra_unix_socket: std::slice::from_ref(&sock),
        extra_deny: &[],
        existing_home_tool_dirs: None,
        existing_app_dirs: None,
        extra_ports: &[],
    ...
    assert!(
        p.contains("(allow file-read* (literal \"/tmp/agent-broker.sock\"))"),
        "unix socket should grant file-read for inode lookup"
    );
    assert!(
        p.contains("(allow network-outbound (literal \"/tmp/agent-broker.sock\"))"),
        "unix socket should grant connect-only outbound"
    );
    assert!(
        !p.contains(
            "(allow network-bind (local unix-socket (subpath \"/tmp/agent-broker.sock\")))"
        ),
        "unix socket flag must not grant bind"
    );
}

#[test]
fn profile_allow_unix_socket_skipped_when_denied() {
    let sock = std::path::PathBuf::from("/tmp/broker.sock");
    let deny = std::path::PathBuf::from("/tmp/broker.sock");
    let p = generate_profile(&ProfileOptions {
        project_dir: std::path::Path::new("/tmp/proj"),
        home_dir: std::path::Path::new("/Users/test"),
        extra_read: &[],
        extra_write: &[],
        extra_unix_socket: &[sock],
        extra_deny: &[deny],
        existing_home_tool_dirs: None,
        existing_app_dirs: None,
        extra_ports: &[],
    ...
    assert!(
        !p.contains("(allow network-outbound (literal \"/tmp/broker.sock\"))"),
        "denied unix socket path must not get connect allow"
    );
    assert!(
        p.contains("Unix socket connect skipped"),
        "profile should note skipped unix socket allows"
    );
}
